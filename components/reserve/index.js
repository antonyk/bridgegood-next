...
//   const { currentStep, room, date } = useSelector(state => state.reservation);
    ...
    function onTimeSlotClick(value) {
        room === 'None' 
            ? null : value.sixLimit === duration
                ? null : value.fourLimit === duration
                    ? null : () => setTimeSlot(value)
    }
    ...
    return (
      ...
      <h3>{(date === null || date < moment().format('YYYY-MM-DD')) ? 'Please choose a possible date' : null}</h3>

              </div>
            </div>

            <div className="col-2 col">
              <h2 className={(date === null || date < moment().format('YYYY-MM-DD')) ? 'h2-grey' : null}>
                Select a Room
              </h2>
              <div className="components">
